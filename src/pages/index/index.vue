...
export default {
  ...
  onShow() {
    ...
    if (this.isWechat) { // H5-微信环境
      if (!tempuid && !token) {
        ...
        this.login('/xxw_shop_auth/ua/social/mp', code) // 先请求微信公众号登录接口
      } else {
        this.queryDataGroup()
      }
    } else {
  ...
  onReachBottom: function() {
    if (this.searchListQuery.pageNum < this.searchListData.totalPage) {
  ...
  methods: {
    ...
    getIndexImgs() {
      uni.showLoading()
      var params = {
        url: '/xxw_shop_business/ua/index_img/list',
        method: 'GET',
        data: {
      ...
      const params = {
        ...
        callBack: res => {
          ...
          if (this.searchListQuery.pageNum === 1) {
            ...
            records = this.categoryProdList
            records = records.concat(res.records[0].spus)
          }
          if (this.searchListQuery.pageNum === this.searchListData.totalPage) {
            this.isLoadAll = true
          }
          this.categoryProdList = records
          uni.hideLoading()
        }
      }
      http.request(params)
    ...
    getPlatformCategoryList() {
      uni.showLoading()
      const params = {
        url: '/xxw_shop_goods/ua/category/category_list',
        method: 'GET',
        data: {
          shopId: 0,
          parentId: 0
